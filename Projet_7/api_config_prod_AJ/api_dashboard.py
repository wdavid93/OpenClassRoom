...
st.title("Exploration des données du client")
...
if option == "Âge":
    ...
    fig = px.histogram(data_train, x=data_train["DAYS_BIRTH"] / -365, nbins=25)
    st.plotly_chart(fig)

elif option == "Revenus":
    st.subheader("Revenus des Clients")
    # fig = px.histogram(data_train, x="revenus")
    fig = px.histogram(data_train, x="AMT_INCOME_TOTAL")
    st.plotly_chart(fig)
