...
def infos_client_id(data_test, id):
    ...
    df = pd.DataFrame.from_dict(dict_infos, orient="index").rename(columns={0: ''})

    # # Ajoutez une ligne vide à la fin
    # empty_row = pd.Series({"Valeur": None}, name="")  # Créez une ligne vide
    # df = df.append(empty_row)  # Ajoutez la ligne vide au DataFrame
    df = df.T
